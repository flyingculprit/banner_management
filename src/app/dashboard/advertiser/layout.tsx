...
import { Search, Image as ImageIcon, Receipt, LogOut, User, Menu, X } from 'lucide-react';
...
export default function AdvertiserLayout({ children }: { children: React.ReactNode }) {
  ...
  const bannerChatsUnread = ((unreadByChannel['advertiser_owner'] || 0) + (unreadByChannel['advertiser_admin'] || 0)) > 0;

  const navItems = [
    { label: 'Explore All Boards', href: '/dashboard/advertiser', icon: Search, unread: false },
    { label: 'Purchased Banners & Dues', href: '/dashboard/advertiser/banners', icon: ImageIcon, unread: bannerChatsUnread },
    { label: 'Payment History', href: '/dashboard/advertiser/payments', icon: Receipt, unread: false },
  ];
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col md:flex-row">
      ...
      <header className="md:hidden flex items-center justify-between px-4 py-3 bg-slate-900 border-b border-slate-800 sticky top-0 z-40">
        <span className="font-bold text-base text-white">Advertiser Hub</span>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden bg-slate-900/95 border-b border-slate-800 p-4 space-y-2 sticky top-[53px] z-30 backdrop-blur-lg">
          ...
          <div className="pt-2 border-t border-slate-800 space-y-2">
            ...
            >
              <User className="w-4 h-4 text-cyan-400" /> Advertiser Profile
            </button>
            <button
              onClick={async () => {
      ...
      <aside className="hidden md:flex w-64 border-r border-slate-800 bg-slate-900/60 p-5 flex-col justify-between shrink-0 h-screen sticky top-0">
        <div>
          <div className="font-bold text-lg text-white mb-8">Advertiser Hub</div>
